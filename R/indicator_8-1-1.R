...
#14-10-0327-01, #14-10-0364-01, #14-10-0083-01
...
Territories <- 
  ...
  select(Year = REF_DATE, 
         ...
         Value = VALUE) %>% 
  mutate(Population = "Total population") %>% 
  mutate(`Age group` = "15 years and over") %>% 
  mutate(Sex = "Both sexes") %>% 
  relocate(Population, .after = "Geography") %>% 
  relocate(`Age group`, .before = "Value") %>% 
  relocate(Sex, .after = "Population")
...
Immigrant <- 
  ...
  filter(REF_DATE >= 2015,
         GEO %in% c("Canada",
                    ...
                    "Alberta",
                    "British Columbia"),
         `Immigrant status` %in% c("Landed immigrants",
...
#Add everything together in one table
...
write_csv(final_data, "CIF/data/indicator_8-1-1.csv", na = "")
